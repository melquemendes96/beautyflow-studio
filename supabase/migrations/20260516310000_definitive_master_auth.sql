-- Master auth definitivo: is_platform_admin + get_auth_panel_context + bootstrap do dono SaaS.
-- Fonte de verdade: public.platform_admins (+ fallback metadata em auth.users).

-- ---------------------------------------------------------------------------
-- is_platform_admin: platform_admins + metadata (role master / is_super_admin)
-- ---------------------------------------------------------------------------
      ...
      OR EXISTS (
        ...
        WHERE u.id = auth.uid()
          AND (
            ...
            OR COALESCE(u.raw_app_meta_data->>'is_super_admin', u.raw_user_meta_data->>'is_super_admin', '')
              IN ('true', 't', '1', 'yes')
          )
      )
    );
...
COMMENT ON FUNCTION public.is_platform_admin() IS
  'True se o usuário autenticado é dono da plataforma (platform_admins ou metadata master).';
...
-- ---------------------------------------------------------------------------
-- get_auth_panel_context: uma chamada para o frontend (sem loop de queries)
-- ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.get_auth_panel_context() IS
  'Contexto do painel: master (platform_admins) e vínculos company_users do auth.uid().';
...
-- ---------------------------------------------------------------------------
-- Garantir master principal (melquemendes96@gmail.com)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- RLS: leitura do próprio registro (authenticated)
-- ---------------------------------------------------------------------------
