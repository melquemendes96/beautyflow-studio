-- Corrige falha ao marcar pagamento como pago no Master.
-- Causas:
-- 1) Triggers de push/marketing usavam COALESCE(enum, ''), incompatível com payment_status/appointment_status
--    → o UPDATE era revertido e master_apply_payment devolvia { ok:false, error:'erro_interno' }.
-- 2) master_apply_payment engolia o erro real; agora devolve detalhe e é idempotente.

BEGIN;

-- ---------------------------------------------------------------------------
-- Triggers: comparação segura de enums
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  THEN
    ...
    EXCEPTION
      WHEN OTHERS THEN
        NULL; -- push não pode bloquear billing
    END;
  END IF;
  RETURN NEW;
END;
...
-- ---------------------------------------------------------------------------
-- Master: aplicar pagamento (2 e 3 argumentos)
-- ---------------------------------------------------------------------------
...
AS $$
BEGIN
  RETURN public.master_apply_payment(p_payment_id, p_months, false);
END;
$$;
...
BEGIN
  ...
  -- Evita acumular várias cobranças pendentes iguais para a mesma renovação
  SELECT pt.id
...
COMMENT ON FUNCTION public.master_apply_payment(uuid, integer, boolean) IS
  'Master: marca payment_transactions como pago, renova assinatura e gera próxima cobrança (platform_admin).';
