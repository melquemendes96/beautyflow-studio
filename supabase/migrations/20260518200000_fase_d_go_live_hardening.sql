-- Fase D: blindagem pré-venda — simulação de pagamento restrita a service_role / platform_admin.
...
BEGIN
  ...
  IF v_role IS DISTINCT FROM 'service_role' AND NOT public.is_platform_admin() THEN
    RETURN json_build_object('ok', false, 'error', 'simulacao_nao_disponivel');
  END IF;
...
REVOKE EXECUTE ON FUNCTION public.company_simulate_payment_outcome(uuid, text) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.company_simulate_payment_outcome(uuid, text) TO service_role;
...
COMMENT ON FUNCTION public.company_simulate_payment_outcome(uuid, text) IS
  'Fase D: apenas service_role ou platform_admin. Tenants não podem simular pagamento aprovado.';
